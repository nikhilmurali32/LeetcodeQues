    ...
    public int minimumEffortPath(int[][] heights) {
        ...
        while(!leastEffort.isEmpty()){
            ...
            for(int[] dir:directions){
                ...
                if(x>=0 && x<m && y>=0 && y<n){
                    int jumpEffort = Math.abs(heights[i][j]-heights[x][y]);
                    int newPathEffort = Math.max(currDist, jumpEffort);
                    if(newPathEffort<minDistance[x][y]){
                        minDistance[x][y] = newPathEffort;
                        leastEffort.add(new int[]{x,y,newPathEffort});
                    }
                }
            }
        }
